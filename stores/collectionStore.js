  ...
  persist(
    (set, get) => ({
      ...
      fetchCollections: async (userId, force = false) => {
        ...
        if (!force && lastFetched && (Date.now() - lastFetched) < 30000 && get().collections.length > 0) {
      ...
      deleteCollection: async (collectionId, userId) => {
        ...
        const deletedCollection = get().collections.find(c => c.id === collectionId);
    ...
    }),
    {
      ...
      partialize: (state) => ({ 
        ...
        collections: state.collections.filter(c => !c.isTemp && !c.isUpdating),
