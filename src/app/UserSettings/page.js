...
export default function UsersSettingsPage() {
  ...
  useEffect(() => {
    rateLimiterRef.current = securityService.getRateLimiter('profile-update', 5, 60000);
  }, []);
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      const sanitizedName = securityService.sanitizeName(authUser.user_metadata?.name || '');
      const sanitizedEmail = securityService.sanitizeEmail(authUser.email || '');
  ...
  const updateProfile = useCallback(async (e) => {
    ...
    // Rate limiting check
    if (rateLimiterRef.current && !rateLimiterRef.current.canProceed(user.id)) {
      setMessage({ text: 'Too many update attempts. Please wait a moment before trying again.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      if (validation.sanitizedData.name && validation.sanitizedData.name !== user.user_metadata?.name) {
        authUpdates.data = { ...user.user_metadata, name: validation.sanitizedData.name };
      }
      
      if (validation.sanitizedData.email && validation.sanitizedData.email !== user.email) {
      ...
      if (Object.keys(authUpdates).length > 0) {
        ...
        if (authUpdates.email) {
          setMessage({ text: 'Email updated. Please sign in again with your new email.', type: 'success' });
          setTimeout(() => {
            signOut();
            router.push('/');
          }, 3000);
      ...
      setMessage({ text: securityService.escapeHtml(error.message || 'Failed to update profile'), type: 'error' });
    } finally {
      setUpdating(false);
    }
  }, [user, name, email, password, confirmPassword, signOut, router, fetchUserData]);
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1 ml-1">
                  Letters, numbers, spaces, hyphens, and periods only (max 50 chars)
                </p>
              </div>
