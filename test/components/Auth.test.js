...
import { Auth } from "../../src/features/auth/hooks/AuthForm";
...
beforeEach(() => {
  ...
  sanitizeEmail.mockImplementation(
...
describe("Auth Component - Validation Tests", () => {
  describe("Form Rendering", () => {
    test("renders sign in form by default", () => {
      ...
      expect(
        screen.getByPlaceholderText("Minimum of 8 Characters is Required"),
      ).toBeInTheDocument();
      expect(
        screen.queryByPlaceholderText("ChooseYourUserName"),
    ...
    test("switches to sign up form when toggle button clicked", async () => {
      ...
      expect(
        screen.getByPlaceholderText("ChooseYourUserName"),
      ).toBeInTheDocument();
      ...
      await waitFor(
        () => {
          const errorMessage = screen.queryByText(
            /Please enter a valid email|valid email address/i,
          );
          expect(errorMessage).toBeInTheDocument();
        },
        { timeout: 2000 },
