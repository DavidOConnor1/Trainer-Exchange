...
export const useUserProfile = () => {
  ...
  const fetchUser = async (id = number) => {
    ...
    try {
      ...
      if (error) {
        ...
        setMessage({
          text: "There was an error retrieving account",
          type: "error",
        });
        return;
      } //end if
  ...
  const removeUser = async (id = number) => {
    //start remove user
    const { error } = await supabase.from("USER TABLE").delete().eq("id", id); //will remove the user if the id matches.

    //inform us of an error if there is one
    if (error) {
      console.log("There was an error trying to remove user: ", error);
      setMessage({
        text: "could not communicate with the database to remove user",
      });
      return false;
    } //end if
    return true;
  ...
  const updateUser = async (id = number) => {
    ...
    if (newPassword && newPassword.trim() !== "") {
      ...
      if (newPassword >= 6) {
        ...
        setMessage({
          text: "password does not match expected character length",
          type: "error",
        });
        setUpdating(false);
        return false;
    ...
    try {
      ...
      if (error) {
        console.log(
          "There was an error trying to update user: ",
          error.message,
        );
      } //end if
      ...
      setMessage({
        text: "There was an error with updating the user profile",
        type: "error",
      });
    } finally {
      setUpdating(false);
